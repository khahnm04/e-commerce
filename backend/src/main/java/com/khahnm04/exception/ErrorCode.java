...
public enum ErrorCode {
    ...
    EMAIL_REQUIRED(2101, "Email cannot be blank", HttpStatus.BAD_REQUEST),
    EMAIL_INVALID_FORMAT(2102, "Email format is invalid", HttpStatus.BAD_REQUEST),
    // phone number
    ...
    PASSWORD_MISSING_UPPERCASE(2302, "Password must contain at least one uppercase letter", HttpStatus.BAD_REQUEST),
    PASSWORD_MISSING_LOWERCASE(2303, "Password must contain at least one lowercase letter", HttpStatus.BAD_REQUEST),
    PASSWORD_MISSING_NUMBER(2304, "Password must contain at least one digit", HttpStatus.BAD_REQUEST),
    PASSWORD_MISSING_SPECIAL_CHAR(2305, "Password must contain at least one special character (!@#$%^&*...)", HttpStatus.BAD_REQUEST),
    // ========== END USER ==========
    ;
