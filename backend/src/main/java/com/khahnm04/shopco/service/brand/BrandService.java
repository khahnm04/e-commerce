...
import com.khahnm04.shopco.exception.DataNotFoundException;
...
import com.khahnm04.shopco.service.cloudinary.CloudinaryService;
...
public class BrandService implements IBrandService {
    ...
    @Override
    public BrandResponse createBrand(BrandRequest request, MultipartFile logoFile) throws Exception {
        Brand existingBrand = brandRepository.findBySlug(request.getSlug());
        if (existingBrand != null) {
            throw new DataNotFoundException("Brand with name " + request.getSlug() + " already exists");
        }
